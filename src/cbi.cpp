...
ClockTreeSynthesizer::~ClockTreeSynthesizer() { cleanup(); }

//----------------------Start calculating, build the clock tree and
//calculate--------------------//

void ClockTreeSynthesizer::buildTree() {
  if (!srcNode || sinks.empty())
    return;
...
std::map<Node *, Node *> ClockTreeSynthesizer::buildRMSTAndGetParentMap(
...
Node *
ClockTreeSynthesizer::findMinKeyNode(const std::vector<Node *> &nodes,
                                     const std::map<Node *, int> &key,
                                     const std::map<Node *, bool> &inMST) {
...
/**
 * @brief Main recursive function that uses RMST-based partitioning to build the
 * clock tree.
 */
void ClockTreeSynthesizer::recursiveBuildWithRMST(Node *parent,
  ...
  // 2. Optimization Pre-check: Check if a single new buffer can handle all
  // targets.
  if (targets.size() <= maxFanout) {
    ...
    // {
    if (calculateClusterWireLength(single_buffer_pos, targets) <= maxLength &&
        manhattanDistance(parent->pos, single_buffer_pos) <= maxLength) {
      // --- IMPLEMENTATION START ---
      // Create one buffer for the whole group
      Node *new_buffer =
          new Node{"B" + std::to_string(buffers.size() + 1), BUFFER};
  ...
  // ORIGINAL: auto [groupA, groupB] = partitionTargetsByBalancedRMSTCut(targets);
  // NEW: Use K-means clustering for better spatial grouping (reduces wirelength)
  auto [groupA, groupB] = partitionTargetsByKMeans(targets);
  ...
  if (!groupA.empty()) {
    ...
    bufferA->pos =
        calculateSkewAwareBufferPosition(parent, groupA, groupB, true);
    ...
    if (distA > limitA) {
      ...
      bufferA->pos.x =
          parent->pos.x + (int)((bufferA->pos.x - parent->pos.x) * scale);
      bufferA->pos.y =
          parent->pos.y + (int)((bufferA->pos.y - parent->pos.y) * scale);
    }
  ...
  if (!groupB.empty()) {
    ...
    // NEW: Use skew-aware positioning that considers sibling subtree depths
    bufferB->pos =
        calculateSkewAwareBufferPosition(parent, groupA, groupB, false);
    ...
    if (distB > limitB) {
      ...
      bufferB->pos.x =
          parent->pos.x + (int)((bufferB->pos.x - parent->pos.x) * scale);
      bufferB->pos.y =
          parent->pos.y + (int)((bufferB->pos.y - parent->pos.y) * scale);
    }
...
/**
 * @brief Partitions a vector of target nodes into two groups based on an RMST
 * cut.
...
ClockTreeSynthesizer::partitionTargetsByBalancedRMSTCut(
  ...
  for (auto const &[node, parent_node] : parent_map) {
    if (parent_node != nullptr) {
      ...
      // Check if this split is more balanced than the best one we've found so
      // far
...
/**
 * @brief NEW: K-means clustering for better spatial grouping
 * Groups sinks by proximity which typically produces tighter clusters
 * than RMST-based cuts, leading to shorter wirelength
 */
std::pair<std::vector<Node *>, std::vector<Node *>>
ClockTreeSynthesizer::partitionTargetsByKMeans(
    const std::vector<Node *> &targets) {
  if (targets.size() <= 1) {
    return {targets, {}};
  }
  ...
  // Initialize 2 centroids using k-means++ style (furthest points)
  Point c1 = targets[0]->pos;
  Point c2 = c1;
  long long maxDist = 0;
  for (const auto &node : targets) {
    long long d = manhattanDistance(c1, node->pos);
    if (d > maxDist) {
      maxDist = d;
      c2 = node->pos;
    }
  ...
  // K-means iterations
  std::vector<Node *> cluster1, cluster2;
  for (int iter = 0; iter < 10; ++iter) {
    ...
    for (Node *node : targets) {
      long long d1 = manhattanDistance(node->pos, c1);
      long long d2 = manhattanDistance(node->pos, c2);
      if (d1 <= d2) {
    ...
    if (cluster1.empty() || cluster2.empty()) {
      // Fallback to balanced split by x-coordinate
      std::vector<Node *> sorted = targets;
      std::sort(sorted.begin(), sorted.end(),
                [](Node *a, Node *b) { return a->pos.x < b->pos.x; });
      size_t mid = sorted.size() / 2;
      return {std::vector<Node *>(sorted.begin(), sorted.begin() + mid),
              std::vector<Node *>(sorted.begin() + mid, sorted.end())};
    }

    // Update step: recalculate centroids
    Point newC1 = calculateBufferPosition(cluster1);
    Point newC2 = calculateBufferPosition(cluster2);

    // Check convergence
    if (newC1.x == c1.x && newC1.y == c1.y && newC2.x == c2.x &&
        newC2.y == c2.y) {
      break;
    }
    c1 = newC1;
...
/**
 * @brief Builds the clock tree using a modified Method of Means and Medians
 * (MMM) algorithm. The recursion terminates based on the class member variables
 * maxFanout and maxLength.
 */
...
long long
ClockTreeSynthesizer::computeSubtreeDepth(const std::vector<Node *> &cluster) {
  if (cluster.empty())
    return 0;
...
void ClockTreeSynthesizer::graph_traverse(
    Node *start_node, std::vector<Node *> &component,
...
long long ClockTreeSynthesizer::calculateClusterWireLength(
    Point center, const std::vector<Node *> &cluster) {
  long long length = 0;
  for (const auto &node : cluster) {
...
Point ClockTreeSynthesizer::calculateBufferPosition(
    const std::vector<Node *> &cluster) {
  if (cluster.empty())
    return {0, 0};
  ...
  for (const auto &node : cluster) {
    ...
    sum_x += node->pos.x;
    sum_y += node->pos.y;
    // std::cout<<"node->pos.x: "<< node->pos.x<< " node->pos.y: "<<
    // node->pos.y<<"\n";
  }
  std::sort(x_coords.begin(), x_coords.end());
  std::sort(y_coords.begin(), y_coords.end());
  // std::cout<<"median x: "<< x_coords[x_coords.size() / 2]<< " median y: "<<
  // y_coords[y_coords.size() / 2]<<"\n"; return {x_coords[x_coords.size() / 2],
  // y_coords[y_coords.size() / 2]}; FIXED: explicit cast to avoid narrowing
  // conversion warning
...
void ClockTreeSynthesizer::calculateArrivalTimes(Node *node,
...
//----------------------After calculating, write out the result and print
//out-----------------//
...
void ClockTreeSynthesizer::populateLevelMap(Node *node) {
  ...
  if (levelMap.find(current_level) == levelMap.end() ||
      std::find(levelMap[current_level].begin(), levelMap[current_level].end(),
                node) == levelMap[current_level].end()) {
...
void ClockTreeSynthesizer::writeOutput(const std::string &filename) {
  ...
  outFile << ".buffer " << buffers.size() << " # number of clock buffers"
          << std::endl;
  for (int i = 0; i < buffers.size(); i++) {
    outFile << buffers[i]->id << " " << buffers[i]->pos.x << " "
            << buffers[i]->pos.y;
    if (i == 0) {
  ...
  outFile << ".level " << levelMap.size() - 1 << " # number of tree levels"
          << std::endl;
    ...
    for (const auto &parent_node : levelMap[i]) {
      if (!parent_node->children.empty()) {
        ...
        for (size_t j = 0; j < parent_node->children.size(); ++j) {
          outFile << parent_node->children[j]->id
                  << (j == parent_node->children.size() - 1 ? "" : " ");
        }
        outFile << "} ";
      }
    }
...
void ClockTreeSynthesizer::printMetrics() const {
  std::cout << "T_max: " << maxArrivalTime << ", T_min: " << minArrivalTime
            << ", W_cbi: " << totalWireLength
            << ", Slack: " << (double)maxArrivalTime / minArrivalTime
            << std::endl;
}

//----------------------After usage, clean up all dynamically allocated
//nodes-----------------//
...
void ClockTreeSynthesizer::cleanup() {
  ...
  if (srcNode) {
    ...
    // This simple cleanup assumes a tree structure where each node is reachable
    // from the source and is only deleted once. For more complex graphs, a more
    // robust cleanup is needed.
