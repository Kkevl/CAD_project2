...
public:
  ...
  ClockTreeSynthesizer(int fanout, int length, Node *src,
                       std::vector<Node *> sinks,
                       std::set<Point> occupiedCoordinates)
      : maxFanout(fanout), maxLength(length), srcNode(src), sinks(sinks),
  ...
  std::map<Node *, Node *>
  buildRMSTAndGetParentMap(const std::vector<Node *> &sinks);
  Node *findMinKeyNode(const std::vector<Node *> &nodes,
...
private:
  ...
  partitionTargetsByRMST(const std::vector<Node *> &targets);
  std::pair<std::vector<Node *>, std::vector<Node *>>
  partitionTargetsByBalancedRMSTCut(const std::vector<Node *> &targets);
  // NEW: K-means based partitioning for better wirelength
  std::pair<std::vector<Node *>, std::vector<Node *>>
  partitionTargetsByKMeans(const std::vector<Node *> &targets);
  ...
  long long calculateClusterWireLength(Point center,
  ...
  Point calculateSkewAwareBufferPosition(Node *parent,
